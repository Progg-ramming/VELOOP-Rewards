...
function App() {
  ...
  const [activeCurrentPrize, setActiveCurrentPrize] = useState(0);
  const [activePreviousWinner, setActivePreviousWinner] = useState(0);
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      ...
      const nextPosition = scroller.scrollLeft + scroller.clientWidth * 0.82;
  ...
  return <div className={styles.appShell}>
    ...
    <header className={styles.navbar}>
      ...
      <div className={styles.navActions}><button className={styles.loginButton} onClick={() => setToast('Login flow is ready for your account integration.')}>Log in</button><button className={styles.profileButton} aria-label="Open profile"><UserRound size={17} /></button><button className={styles.menuButton} onClick={() => setMobileNav((value) => !value)} aria-label="Toggle navigation"><Menu size={20} /></button></div>
    </header>
    ...
    <main id="top">
      <section className={styles.hero} id="giveaway">
        <div className={styles.heroCopy}>
          ...
          <div className={styles.heroActions}><a className={styles.primaryButton} href={primaryAction}>{actionLabel} <ArrowRight size={17} /></a><a className={styles.textButton} href="#how-it-works">See how it works <ChevronRight size={15} /></a></div>
      ...
      <section className={styles.footerCta}><div><span className={styles.sectionKicker}>Your next good thing</span><h2>Ready to make your entry?</h2><p>Join the September reward drop before entries close.</p></div><a className={styles.primaryButton} href={primaryAction}>{actionLabel} <ArrowRight size={17} /></a></section>
    </main>
